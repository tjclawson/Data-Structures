class Heap:
    ...
    def _bubble_up(self, index):
        ...
        if parent_index >= 0 and self.comparator(self.storage[index], self.storage[parent_index]):
    ...
    def _sift_down(self, index):
        bigger_child = self._get_correct_child(index)
        if bigger_child is not None and self.comparator(bigger_child, self.storage[index]):
            bigger_child_index = self.storage.index(bigger_child)
            self.storage[bigger_child_index], self.storage[index] = self.storage[index], self.storage[
                bigger_child_index]
            return self._sift_down(self.storage.index(bigger_child) + 1)

    def _get_correct_child(self, index):
        right_child = None
        left_child = None
        ...
        if right_child is not None and left_child is not None:
            if self.comparator(right_child, left_child):
                return right_child
            else:
                return left_child
